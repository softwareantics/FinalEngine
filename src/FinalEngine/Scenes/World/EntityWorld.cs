...
using FinalEngine.Scenes.Exceptions;
...
internal sealed class EntityWorld : IEntityWorld
{
    ...
    public event EntitySystemAddedEventHandler? OnEntitySystemAdded;

    public event EntitySystemRemovedEventHandler? OnEntitySystemRemoved;
    ...
    public Entity AddEntity<TFactory>()
        where TFactory : IEntityFactory
    {
        return this.AddEntity(this.factoryResolver.GetEntityFactory<TFactory>().CreateEntity());
    }
    ...
    public void AddSystem(EntitySystemBase system)
    {
        ...
        this.OnEntitySystemAdded?.Invoke(this, system);
    }
    ...
    public void RemoveSystem(Type type)
    {
        ...
        for (int i = this.systems.Count - 1; i >= 0; i--)
        {
            if (this.systems[i].GetType() == type)
            {
                ...
                this.OnEntitySystemRemoved?.Invoke(this, system);
